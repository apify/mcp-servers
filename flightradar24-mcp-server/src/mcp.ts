 ...
 */
// + add (optional) path if you want to set cwd

export async function getMcpProxyClient(command: string[]): Promise<Client> {
    // Build a string-only env for the child process
    const childEnv: Record<string, string> = Object.fromEntries(
        Object.entries(process.env).flatMap(([k, v]) => (v == null ? [] : [[k, String(v)]]))
    );

    // Ensure FR24 defaults if URL unset (key remains required)
    if (!childEnv.FR24_API_URL) {
        childEnv.FR24_API_URL = "https://fr24api.flightradar24.com";
    }

    const transport = new StdioClientTransport({
        command: command[0],
        args: command.slice(1),
        env: childEnv,                 // ✅ type-safe env
        cwd: process.cwd(),            // good for relative paths
    });
