...
like tool calls or resource access.
"""
...
async def charge_mcp_operation(
    ...
    try:
        ...
        logger.info(f'Charged for event: {event_name} (count={count})')
    except Exception:
...
async def create_gateway(  # noqa: PLR0915
    ...
    Args:
        ...
        tool_whitelist: Optional dict mapping tool names to (event_name, default_count) tuples.
                        ...
                        If None, all tools are allowed and charged under TOOL_CALL.
    """
    ...
    if capabilities.prompts:
        ...
        async def _get_prompt(req: types.GetPromptRequest) -> types.ServerResult:
            # If you wish to charge for prompt retrieval later, uncomment:
    ...
    # ----------------------
    # Resources (no charging by default)
    # ----------------------
    ...
    # ----------------------
    if capabilities.resources:
        logger.debug('Capabilities: adding Resource Subscriptions...')
    ...
    # ----------------------
    # Tools (whitelisting + charging per call)
    # ----------------------
    if capabilities.tools:
        ...
        async def _list_tools(_: Any) -> types.ServerResult:
            ...
            if tool_whitelist:
                authorized = [tool for tool in tools.tools if tool.name in tool_whitelist]
                tools.tools = authorized

            # NOTE: No charge for listing tools in the simplified billing model
        ...
        async def _call_tool(req: types.CallToolRequest) -> types.ServerResult:
            ...
            try:
                ...
                # Determine charging event and count
                ...
                # Charge for the tool call (single flat event)
    ...
    # ----------------------
    # Progress notifications & completion
    # ----------------------
