# ![Apify Actor Badge](https://badges.apify.com/pubmed-mcp-server.svg)

# PubMed MCP Server (Apify Actor)

This Apify Actor exposes the PubMed MCP Server for AI assistants and automation via the Model Context Protocol (MCP).
...
2. MCP client config (stdio):
     ...
     ```
3. [MCP Inspector](https://mcp.apify.com) for testing.

## Pay-per-event mapping
See `.actor/pay_per_event.json` for billable tool events.

## Credits
Based on [@JackKuo666/pubmed-mcp-server](https://github.com/JackKuo666/PubMed-MCP-Server).
