...
def generate_pubmed_search_url(
    ...
    params = {"db": "pubmed", "term": query, "retmax": num_results, "retmode": "xml"}
    ...
    if response.status_code == 200:
        ...
        if article is not None:
            ...
            pub_date = (
                ...
                else "No publication date available"
            )
...
def download_full_text_pdf(pmid):
    ...
    if response.status_code != 200:
        print(
            f"Error: Unable to fetch article data (status code: {response.status_code})"
        )
        return (
            f"Error: Unable to fetch article data (status code: {response.status_code})"
        )
    ...
    if pmc_id is None:
        ...
        return (
            f"No PMC ID found for PMID: {pmid}"
            + "\n"
    ...
    if "This article is available under a" not in pmc_response.text:
        ...
        print(f"You can check the article availability at: {pmc_url}")
        return (
            f"The article doesn't seem to be fully open access."
            + "\n"
            + f"You can check the article availability at: {pmc_url}"
        )
...
def search_advanced(term, title, author, journal, start_date, end_date, num_results):
    # 生成搜索 URL
    search_url = generate_pubmed_search_url(
        term=term,
        title=title,
        author=author,
        journal=journal,
