...
async def get_cash_flow_statements(
    ...
    # Fetch data from the API
    url = (
        f'{FINANCIAL_DATASETS_API_BASE}/financials/cash-flow-statements/?ticker={ticker}&period={period}&limit={limit}'
    )
    data = await make_request(url)
...
async def get_historical_stock_prices(
    ...
    # Fetch data from the API
    url = (
        f'{FINANCIAL_DATASETS_API_BASE}/prices/?ticker={ticker}&interval={interval}'
        f'&interval_multiplier={interval_multiplier}&start_date={start_date}&end_date={end_date}'
    )
    data = await make_request(url)
...
async def get_historical_crypto_prices(
    ...
    Args:
        ticker: Ticker symbol of the crypto currency (e.g. BTC-USD). The list of available crypto
            tickers can be retrieved via the get_available_crypto_tickers tool.
        start_date: Start date of the price data (e.g. 2020-01-01)
        end_date: End date of the price data (e.g. 2020-12-31)
        interval: Interval of the price data (e.g. minute, hour, day, week, month)
        interval_multiplier: Multiplier of the interval (e.g. 1, 2, 3)
