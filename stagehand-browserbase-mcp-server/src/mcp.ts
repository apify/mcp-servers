...
export async function getMcpServer(command: string[], options?: {
 ...
 const env = {
    ...
    const proxyClient = await getMcpProxyClient(command, env);
 ...
 */
export async function getMcpProxyClient(command: string[], env: Record<string, string | undefined>): Promise<Client> {

  // Create a new, clean object that will only contain keys with actual string values.
    ...
    const transport = new StdioClientTransport({
        ...
        env: cleanEnv, // Use the new, clean object here
    });
